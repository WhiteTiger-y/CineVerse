...
export default function ChatPage() {
  ...
  useEffect(() => {
    const loadMessages = async () => {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/chat/messages?session_id=${encodeURIComponent(
          activeSessionId
        )}`,
        { headers: { Authorization: `Bearer ${user.access_token}` } }
      );
      if (res.ok) {
        ...
        setMessages(
          mapped.length
            ? mapped
            : [{ sender: "bot", text: "New chat started. How is your day going?" }]
        );
      } else {
        setMessages([
          { sender: "bot", text: "New chat started. How is your day going?" },
  ...
  const handleNewChat = async () => {
    ...
    if (res.ok) {
      ...
      setMessages([
        { sender: "bot", text: "New chat started. How are you feeling today?" },
      ]);
      const sessionsRes = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/chat/sessions`,
        { headers: { Authorization: `Bearer ${user.access_token}` } }
  ...
  const handleDeleteChat = async (sid) => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/chat/session/${encodeURIComponent(sid)}`,
      {
        method: "DELETE",
        headers: { Authorization: `Bearer ${user.access_token}` },
      }
